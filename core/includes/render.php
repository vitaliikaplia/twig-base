...
function get_allowed_templates($with_others = false) {
    ...
    if(!empty($with_others)) {
        // add templates from views/others/ directory
        $others = glob(ABSPATH . DS . TWIG_VIEWS_DIRNAME . DS . 'others' . DS . '*.twig');
        if(!empty($others)) {
            foreach($others as $other) {
                $templates[] = basename($other, '.twig');
            }
        }
    }
    return $templates;
...
function get_allowed_others_templates() {
    $templates = glob(ABSPATH . DS . TWIG_VIEWS_DIRNAME . DS . 'others' . DS . '*.twig');
